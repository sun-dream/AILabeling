**核心气质**：这是一个以**信任感与专业性**为核心的工程服务类网站。视觉上呈现为一种 “权威且有温度” 的专业承包商形象。通过高留白的极简主义排版与沉稳的色彩搭配，将紧急火灾修复这一沉重话题转化为高效、可靠的叙事流。文字密度适中，强调阅读逻辑，旨在为处于危机中的客户提供清晰的行动指引。

* * *
### **2. 页面结构与流 (Structure & Flow)**

-   **多维导航体系 (Mega Menu System)** ：顶部采用双层结构，顶层为紧急联系信息流，底层为带有深度嵌套层级的服务导航，确保用户在任何位置都能一键启动紧急拨号。
-   **全屏沉浸式 Hero 区（表单集成布局）** ：Hero 区域采用非对称流体布局，左侧以巨大字重的标题定调，右侧直接嵌入 “快速咨询” 入口，缩短用户从 “受灾认知” 到 “寻求帮助” 的路径。
-   **网格结构化服务板块 (Bento Grid Style)** ：服务项目采用多列网格容器，每个模块配以微型引导标识，将复杂的技术服务内容拆解为易于消化的信息碎片。
-   **信任背书模块 (Social Proof Zone)** ：在页面中后段穿插灰度处理的合作伙伴 Logo 画布，平衡页面视觉重心。

* * *

### **3. 交互与动效编排 (Interaction & Motion Choreography)**

-   **紧急呼叫磁吸感 (Floating CTA Interaction)** ：右下角悬浮的两个圆形通讯触点（WhatsApp / 电话）带有轻微的呼吸律动，并在滚动时保持绝对层级，为用户提供即时的心理安全感。
-   **信息层级揭示**：页面滚动时，各板块标题与正文采用 ** 平滑上浮（Smooth Upward Reveal）** 的次序进场，体现工程行业的严谨与步调一致。
-   **表单焦点反馈**：输入框在获取焦点时，边框颜色发生细腻的色阶过渡，并伴随占位符文案的轻量位移，提升交互的确定性。
-   **FAQ 柔性展开**：常见问题采用手风琴式交互，高度变化带有明显的阻尼感，避免内容闪现带来的视觉突兀。
...
-   **色彩规范**：
    ...
    -   **核心强调色 (Accent Red)** ：设定为 **熟赭色 (#8E312A)** ，用于 `background-color` (按钮) 和 `text-color` (标题高亮)，象征力量、专业与紧急状态下的警示。
    -   **主背景色 (Background)** ：使用 **纯净白 (#FFFFFF)**  与极浅的 **烟雾灰 (#F9FAFB)**  交替，通过色块区分逻辑区域。
    -   **文本主色 (Primary Text)** ：设定为 **深海蓝黑 (#101828)** ，确保最高的 WCAG 阅读对比度。
    -   **线条与边界 (Border)** ：使用 **晨曦灰 (#EAECF0)** ，以轻量化的纹理感定义容器边界。

-   **排版**：采用 **Inter** 配合 **Maven Pro** 双字体系统。Hero 标题字重设为 600-700，通过负间距处理（-0.02em）提升视觉凝聚力。
...
-   **Hero 区域**：
    ...
    -   **主标题**：Expert Fire Damage Restoration in **New York**
    -   **副标题**：When fire strikes, Zicklin Contracting restores your property with expert care...

-   **咨询转化区 (Consultation Zone)** ：

    -   **区块标签**：CONTACT US
    -   **表单引导**：Request a Free Consultation
    -   **微文案**：Email (you@company.com), Zip Code, Project feedback.

-   **核心竞争优势**：

    -   Expertise, Full-Service Restoration, Advanced Technology, Customer Satisfaction.

-   **服务明细项**：

    -   Emergency Response (24/7 Available), Smoke and Soot Removal, Structural Cleaning, Insurance Claim Assistance.

-   **页脚信息**：

    -   包含 37 年家族经营历史描述、五个行政区的详细办公地址、DCA 执照号。
* * *

### **6. 还原指令摘要 (Implementation Priorities)**
1.  **首屏响应优先级**：Hero 区域的 `background-color` 图纸底纹需通过延迟加载确保标题文字优先呈现。
1.  **色彩精准度**：必须强制按钮与交互状态使用  **#8E312A**，不得出现色差，以维持品牌的专业调性。
1.  **无障碍语义**：由于页面包含大量 Heading，必须确保视觉上的样式层级（H1/H2/H3）与 `accessibility.json` 中的语义层级完全一致。
